...
function ArtistContent() {
  const [displayedArtists, setDisplayedArtists] = useState<Artist[]>(
    sampleArtists.slice(0, 8)
  );
  const [filteredArtists, setFilteredArtists] =
    useState<Artist[]>(sampleArtists);
  ...
  const handleSort = (value: string) => {
    ...
    switch (value) {
      ...
      case 'followers':
        sorted.sort((a, b) => {
          const aFollowers =
            parseFloat(a.followers.replace(/[KM]/g, '')) *
            (a.followers.includes('M') ? 1000000 : 1000);
          const bFollowers =
            parseFloat(b.followers.replace(/[KM]/g, '')) *
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
          ...
          <div className="text-sm text-gray-500 font-poppins dark:text-gray-400">
            Showing {displayedArtists.length} of {filteredArtists.length}{' '}
            artists
          </div>
        </div>
        ...
        <div className="grid  grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {displayedArtists.map((artist) => (
            <Card
              key={artist.id}
              className="group border-none hover:shadow-lg transition-all duration-300 cursor-pointer"
            >
              <CardContent className="p-4 border-none">
                ...
                <div className="space-y-2 text-center">
                  <h3 className="font-semibold text-lg font-poppins truncate text-white">
                    {artist.name}
                  </h3>
